...
export const escucharMisTrabajosOperario = (userData) => {
    ...
    const renderOperario = () => {
        ...
        listDiv.innerHTML = todas.map(t => {
            ...
            const btnCompletar = t.estado !== 'completado'
                ? `<button onclick="marcarCompletado('${t.jobId}')" class="btn btn-warning btn-small" style="margin-top: 16px;"><i class="fas fa-check"></i> Completar Tarea</button>`
                : `<div style="margin-top: 16px; color: #22c55e; font-weight: 600;"><i class="fas fa-check-circle"></i> Tarea Completada</div>`;
            return `
            <div style="background: white; padding: 24px; border-radius: 12px; margin-bottom: 20px; border-left: 3px solid ${t.estado === 'completado' ? '#22c55e' : 'var(--warning)'}; border: 1px solid var(--border); box-shadow: var(--shadow-sm);">
                <div style="display: flex; justify-content: space-between; margin-bottom: 12px;">
                    <div>
                        <strong style="color: var(--text-main); font-size: 16px;">${t.categoria}</strong>
                        <span style="font-size: 12px; color: var(--text-muted); display: block;">Cliente: ${t.clienteNombre || 'N/A'}</span>
                    </div>
                    <span class="badge ${badgeClass}">${t.estado.toUpperCase()}</span>
                </div>
                <p style="font-size: 14px; color: var(--text-muted); line-height: 1.5; padding: 12px; background: var(--bg-app); border-radius: 8px;">${t.descripcion}</p>
                ${btnCompletar}
            </div>`;
        }).join('');
    };
    ...
    const unsubActivos = onSnapshot(
        query(collection(db, "trabajos"), where("operarioId", "==", userData.uid), where("estado", "in", ["solicitado", "asignado"])),
...
export const marcarCompletado = async (jobId) => {
    try {
        await updateDoc(doc(db, "trabajos", jobId), { estado: 'completado' });
        showToast("¡Buen trabajo! Tarea completada.", "success");
    } catch (err) {
